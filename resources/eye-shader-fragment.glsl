...
void main()
{
	...
	// handle irises
	vec3 VP = vec3( lookAtPosition ) - ecPosition3;
	float irisOffset = 0.8;
	float irisLimit = irisOffset + ((1.0 - irisSize) * (1.0 - irisOffset));

	VP = normalize(VP);
	float nDotVP = dot( normal, VP );
	
	if (nDotVP > irisOffset && nDotVP < irisLimit) {
